...
CREATE POLICY "Admins can upload viewpoint images"
  ...
  WITH CHECK (bucket_id = 'viewpoint-images' AND public.has_role(auth.uid(), 'admin'::public.app_role));

CREATE POLICY "Admins can update viewpoint images"
  ON storage.objects FOR UPDATE
  USING (bucket_id = 'viewpoint-images' AND public.has_role(auth.uid(), 'admin'::public.app_role));
