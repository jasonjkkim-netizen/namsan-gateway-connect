...
export default function Home() {
  ...
  const navLinks = [{
    href: '#about',
    label: language === 'ko' ? '회사소개' : 'About Us'
  }, {
    href: '#philosophy',
    label: language === 'ko' ? '투자철학' : 'Philosophy'
  }, {
    href: '#contact',
    label: language === 'ko' ? '연락처' : 'Contact'
  }];
  const values = [{
    icon: Shield,
    titleKo: '안정적 수익',
  ...
  return <div className="min-h-screen flex flex-col bg-background">
      ...
      <header className="fixed top-0 left-0 right-0 z-50 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/80 border-b border-border">
        <div className="container flex items-center justify-between h-20">
          <div className="flex items-center gap-3">
            ...
            <span className="text-xl font-semibold text-primary hidden sm:inline tracking-wide font-serif">
              NAMSAN KOREA
            </span>
          </div>
          ...
          <nav className="hidden md:flex items-center gap-8">
            {navLinks.map(link => <a key={link.href} href={link.href} className="text-sm font-medium text-muted-foreground hover:text-primary transition-colors tracking-wide">
                {link.label}
              </a>)}
            <Link to="/login">
          ...
          <div className="flex items-center gap-3 md:hidden">
            <LanguageToggle />
            <button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="p-2 text-foreground hover:bg-muted rounded-md transition-colors" aria-label={isMobileMenuOpen ? 'Close menu' : 'Open menu'}>
        ...
        {isMobileMenuOpen && <div className="md:hidden absolute top-full left-0 right-0 bg-background border-b border-border shadow-lg animate-fade-in">
            <nav className="container py-4 flex flex-col gap-2">
              {navLinks.map(link => <a key={link.href} href={link.href} className="px-4 py-3 text-base font-medium text-foreground hover:bg-muted rounded-md transition-colors" onClick={() => setIsMobileMenuOpen(false)}>
                  {link.label}
                </a>)}
      ...
      {/* Hero Section */}
      <section className="relative flex-1 flex items-center justify-center pt-32 pb-24 px-6 min-h-[80vh]" style={{
        ...
        <div className="container max-w-5xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 gap-12 items-center">
            <div className="space-y-6 text-muted-foreground leading-relaxed">
              <p className="text-lg">
                {language === 'ko' ? '남산코리아는 아시아 지역의 고액자산가 고객을 위한 자산관리(Wealth Management) 서비스를 제공하고 있습니다.' : 'Namsan Korea provides Wealth Management Services for high-net-worth clients in the Asia region.'}
              </p>
              
              <p className="text-lg">
                {language === 'ko' ? '엄격한 리스크 관리와 혁신적인 투자 전략을 통해, 경험이 풍부한 투자팀이 고객에게 보수적이면서도 시장을 상회하는 성과를 제공하기 위해 노력합니다.' : 'Through rigorous risk management and innovative investment strategies, our experienced investment team strives to deliver conservative yet above-market results to our clients.'}
              </p>

              <p className="text-lg font-medium text-foreground">
                {language === 'ko' ? '오랜 신뢰와 견고한 파트너십을 바탕으로, 지속적인 투자 성과를 함께 만들어 갑니다.' : 'Built on enduring trust and lasting partnerships, we deliver consistent investment returns together.'}
              </p>
            </div>
            
            <div className="grid grid-cols-2 gap-6">
              {values.map((value, index) => <div key={index} className="p-6 rounded-lg bg-secondary/50 border border-border hover:border-accent/50 transition-colors">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            <div className="p-8 bg-background rounded-lg border border-border">
              ...
              <p className="text-muted-foreground text-sm">
                {language === 'ko' ? '단기 수익보다 장기적인 자산 증식에 초점을 맞춥니다' : 'We focus on long-term asset growth rather than short-term gains'}
              </p>
            </div>
            ...
            <div className="p-8 bg-background rounded-lg border border-border">
              ...
              <p className="text-muted-foreground text-sm">
                {language === 'ko' ? '철저한 분석과 분산투자로 리스크를 최소화합니다' : 'We minimize risk through thorough analysis and diversification'}
              </p>
            </div>
            ...
            <div className="p-8 bg-background rounded-lg border border-border">
              ...
              <p className="text-muted-foreground text-sm">
                {language === 'ko' ? '정기적인 리포트와 소통으로 투명성을 유지합니다' : 'We maintain transparency through regular reports and communication'}
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="grid md:grid-cols-3 gap-8 text-center">
            <div className="p-8 rounded-lg bg-secondary/30 border border-border">
              ...
              <p className="text-foreground">
                {language === 'ko' ? '서울특별시 강남구 역삼동 705-22' : '705-22 Yeoksam Dong, Gangnam Gu'}
                <br />
